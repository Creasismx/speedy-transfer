...
def send_booking_email(order, request, booking_id=None, test_recipients=False):
    """
    Sends a booking confirmation email to the guest and/or test recipients.
    
    Args:
        order: The order dictionary containing booking details
        request: The HTTP request object
        booking_id: The booking ID from the database (optional)
        test_recipients: If True, only send to admin recipients
    """
    try:
        # Log email attempt
        ...
        booking_reference = f"SPEEDY-{booking_id:06d}" if booking_id else "PENDING"
        ...
        <body>
            <div class="container">
                ...
                <div class="section">
                    <h3>Customer</h3>
                    <table>
                        <tr><th>Booking Reference</th><td><strong>{booking_reference}</strong></td></tr>
                ...
                <div class="section">
                    <h3>Trip Details</h3>
                    <table>
                        <tr><th>Trip Type</th><td>{trip_type_display}</td></tr>
                        <tr><th>Number of Passengers</th><td>{people}</td></tr>
                        <tr><th>Pickup Location</th><td>{pickup.get('location_name','')}</td></tr>
                        <tr><th>Pickup Date & Time</th><td>{pickup.get('datetime','')}</td></tr>
                        <tr><th>Dropoff Location</th><td>{dropoff.get('location_name','')}</td></tr>
                        {f"<tr><th>Return Pickup Location</th><td>{(return_trip or {}).get('pickup_location_name','')}</td></tr>" if return_trip else ''}
                        {f"<tr><th>Return Date & Time</th><td>{(return_trip or {}).get('datetime','')}</td></tr>" if return_trip else ''}
                        {f"<tr><th>Return Dropoff Location</th><td>{(return_trip or {}).get('dropoff_location_name','')}</td></tr>" if return_trip else ''}
                        <tr><th>Vehicle Type</th><td>{car_type_label}</td></tr>
                    </table>
                </div>
                <div class="section">
                ...
                <div class="section">
                    ...
                    <table>
                        <tr><th>Total Amount</th><td><strong>{total:.2f} USD</strong></td></tr>
                        <tr><th>Payment Method</th><td>{order.get('payment_method', 'N/A')}</td></tr>
                    </table>
                </div>
                <div class="footer">
                    ...
                    <ul style="margin: 10px 0; padding-left: 20px;">
                        ...
                        <li>If you need to make changes or cancel, please contact us at least 24 hours in advance</li>
                        <li>For any questions or assistance, please contact us using your booking reference: <strong>{booking_reference}</strong></li>
                    </ul>
                    <p style="margin-top: 20px;">Thank you for choosing Speedy Transfer. We look forward to serving you!</p>
                    <p style="margin-top: 10px; color: #999; font-size: 12px;">This is an automated confirmation email. Please do not reply to this message.</p>
                </div>
            </div>
        </body>
        </html>
        """
        # Build text body in English
        text_parts = [
            "Booking Confirmation\n",
            f"Dear {guest_name},\n\n",
            "Thank you for choosing Speedy Transfer! Your booking has been confirmed.\n\n",
        ...
        if guest_address:
            text_parts.append(f"Address: {guest_address}\n")
        if guest_city:
            text_parts.append(f"City: {guest_city}\n")
        if guest_state:
            text_parts.append(f"State/Province: {guest_state}\n")
        if guest_zip:
            text_parts.append(f"ZIP Code: {guest_zip}\n")
        if guest_country:
            text_parts.append(f"Country: {guest_country}\n")
        if guest_company:
            text_parts.append(f"Company: {guest_company}\n")
        if guest_dob:
            text_parts.append(f"Date of Birth: {guest_dob}\n")
        if guest_passport:
            text_parts.append(f"Passport Number: {guest_passport}\n")
        if emergency_name or emergency_phone:
            text_parts.append(f"Emergency Contact: {emergency_name} - {emergency_phone}\n")
        if special_instructions:
            text_parts.append(f"Special Instructions: {special_instructions}\n")
        if additional_notes:
            text_parts.append(f"Additional Notes: {additional_notes}\n")
        
        text_parts.extend([
            "\nTRIP DETAILS:\n",
            f"Trip Type: {trip_type_display}\n",
            f"Number of Passengers: {people}\n",
            f"Pickup Location: {pickup.get('location_name','')}\n",
            f"Pickup Date & Time: {pickup.get('datetime','')}\n",
            f"Dropoff Location: {dropoff.get('location_name','')}\n",
        ])
        
        if return_trip:
            text_parts.append(f"Return Pickup Location: {return_trip.get('pickup_location_name','')}\n")
            text_parts.append(f"Return Date & Time: {return_trip.get('datetime','')}\n")
            text_parts.append(f"Return Dropoff Location: {return_trip.get('dropoff_location_name','')}\n")
        
        text_parts.extend([
            f"Vehicle Type: {car_type_label}\n",
            "\nPAYMENT SUMMARY:\n",
            ...
            f"- For any questions or assistance, please contact us using your booking reference: {booking_reference}\n",
            "\nThank you for choosing Speedy Transfer. We look forward to serving you!\n\n",
        ...
        try:
            ...
            print(f"⚠️ Email error type: {type(email_error).__name__}")
            import traceback
            print("⚠️ Full error traceback:")
            traceback.print_exc()
