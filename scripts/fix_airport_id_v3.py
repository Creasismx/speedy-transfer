...
def main():
    ...
    try:
        ...
        # Correcting the data via raw SQL with MySQL NO_AUTO_VALUE_ON_ZERO
        sql_code = """
from django.db import connection
try:
    with connection.cursor() as cursor:
        # Enable NO_AUTO_VALUE_ON_ZERO for the session
        cursor.execute("SET SESSION sql_mode = CONCAT(@@sql_mode, ',NO_AUTO_VALUE_ON_ZERO')")
        print('SQL MODE updated for session.')

        # 1. Remove Any AEROPUERTO zones that are not ID 0
        ...
        # 3. Ensure Hotel ID 0 exists and is linked
    ...
    print(f'Error: {e}')
"""
        stdin, stdout, stderr = ssh.exec_command("cat > /tmp/fix_airport_id_v2.py")
        stdin.write(sql_code)
        stdin.close()
        stdout.channel.recv_exit_status()

        status, out, err = run_command(ssh, "cd /var/www/speedy-transfer && /var/www/speedy-transfer/venv/bin/python manage.py shell < /tmp/fix_airport_id_v2.py")
        print("\n--- Command Output ---")
        print(out)
